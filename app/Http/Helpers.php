...
if (!function_exists('check_free_shipping_api')) {
    function check_free_shipping_api($city_id, $user_id)
    {
        $subtotal = 0;
        $tax = 0;
        $total = 0;
        ...
        $counter = 0;
           ...
           $total = $tax + $subtotal;
            ...
            $business_settings = BusinessSetting::where('type', 'min_order_cost')->first();
            if($city){
            if($city->free){
                if($business_settings && $total >= $business_settings->value){
                    return 0;
                }else{
                    foreach ($carts as $key => $cartItem) {
                            $subtotal += $cartItem['price'] * $cartItem['amount'];
                            $tax += $cartItem['tax'] * $cartItem['amount'];
                        if(check_free_shipping_product($cartItem['product_id'])){
                            $shipping[] = 0;
                        }else{
                            $counter++;
                            $shipping[] = $city->delivery_price;
                        }
                    }
                    $counter?:$counter=1;
                    return array_sum($shipping)/$counter;
                }
            }else{
                                    foreach ($carts as $key => $cartItem) {
                            $subtotal += $cartItem['price'] * $cartItem['amount'];
...
if (!function_exists('check_free_shipping_cart_api')) {
    function check_free_shipping_cart_api($city_id, $user_id)
    {
        $subtotal = 0;
        $tax = 0;
        $total = 0;
...
if (!function_exists('check_free_shipping')) {
    function check_free_shipping($city_id)
    {
        $subtotal = 0;
        $tax = 0;
        $total = 0;
            foreach (Session::get('cart_'.get_country()->code) as $key => $cartItem) {
...
if (!function_exists('check_free_shipping_front')) {
    function check_free_shipping_front($city_id)
    {
        $subtotal = 0;
        $tax = 0;
        $total = 0;
            ...
            if($city->free){
                if($business_settings && $total >= $business_settings->value){
                    ...
                    foreach (Session::get('cart_'.get_country()->code) as $key => $cartItem) {
                        $subtotal += $cartItem['price'] * $cartItem['quantity'];
                        $tax += $cartItem['tax'] * $cartItem['quantity'];
                        if(check_free_shipping_product($cartItem['id'])){
                            $shipping[] = 0;
                        }else{
                            $counter++;
                            $shipping[] = $city->delivery_price;
...
if (!function_exists('check_free_shipping_product')) {
    function check_free_shipping_product($product_id)
{
...
if (!function_exists('checkUrlSlider')) {
    function checkUrlSlider($url,$lang)
    {
        ...
        if(Str::contains($url, 'search')){
            $type = substr($url, strpos($url, '?')+1 , (strpos($url, '='))-(strpos($url, '?')+1));
            if($type == 'subsubcategory'){
                $subsubcategory = SubSubCategory::where('slug',Str::after($url,'='))->first();
                if($subsubcategory){
                    $type_id = $subsubcategory->id;
                    $name = $subsubcategory->$column;
                ...
                $subcategory = SubCategory::where('slug',Str::after($url,'='))->first();
                if($subcategory){
        ...
        }else if(Str::contains($url, 'product')){
            $type = 'product';
            $slug = substr($url, strpos($url, 'product/')+8 , (strpos($url, '?'))-(strpos($url, 'product/')+8));
